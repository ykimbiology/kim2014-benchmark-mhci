...
def reduce_seq(peptide_list, d_count_neighbour, sim_cutoff, debug=False):
    """
    ...
    for peptide_i in sorted_seq:
        ...
        for peptide_j in forward_seq:
            if (is_similar(peptide_i,peptide_j, sim_cutoff=sim_cutoff) == True):
                match = True
                break;
        if match == False:
            forward_seq.append(peptide_i)
    is_error = check_seq_list(forward_seq, sim_cutoff=sim_cutoff)
    assert is_error == False
    return forward_seq

def get_peptide_list_reduced(peptide_list, sim_cutoff=0.80, debug = False):
    """
    Returns a list of peptide_list_sr such that no two **similar** peptides.
    ALGORITHM: 
    """
    d_count_neighbour = count_neighbour(peptide_list, sim_cutoff=sim_cutoff)
    print d_count_neighbour

    peptide_list_reduced = []
    if len(peptide_list) > 0: peptide_list_reduced = reduce_seq(peptide_list, d_count_neighbour, sim_cutoff=sim_cutoff)
